namespace Chess.Model.Rule
{
    ...
    /// <summary>
    /// Represents the chess960 rulebook.
    /// </summary>
    public class Chess960Rulebook : IRulebook
    {
        /// <summary>
        /// Represents the check rule of a chess960 game which happens to be the same as the standard rule.
        /// </summary>
        private readonly CheckRule checkRule;

        /// <summary>
        /// Represents the end rule of a chess960 game which happens to be the same as the standard rule.
        /// </summary>
        private readonly EndRule endRule;

        /// <summary>
        /// Represents the movement rule of a chess960 game which is currently the same as the standard rule.
        /// however, the chess960 rulebook alters castling rules, so this might change in the future.
        /// </summary>
        ...
        public ChessGame CreateGame()
        {
            ...
            do
            {
                // Shuffle the pieces to create a random base line
                pieces = pieces.OrderBy(_ => random.Next()).ToList();

                // verify that the chess960 rules are followed
                // The bishops must be on opposite color squares.
                int biship1Index = pieces.FindIndex(p => p is "Bishop");
                int bishop2Index = pieces.FindLastIndex(p => p is "Bishop");

                // The king must be between the two rooks.
                int rook1Index = pieces.FindIndex(p => p is "Rook");
                int kingIndex = pieces.FindIndex(p => p is "King");
                int rook2Index = pieces.FindLastIndex(p => p is "Rook");

                // if the bishops are both on even/odd then they are on the same color square,
                if (biship1Index % 2 == bishop2Index % 2)
                {
                    isValid = false;
                }
                else if (rook1Index > kingIndex || rook2Index < kingIndex)
                {
                    isValid = false;
                }
                else
            ...
            {
                // Create a new list of pieces for the base line based on the shuffled pieces
                var baseLinePieces = pieces.Select<string, ChessPiece>(p =>
                {
                    return p switch
                    {
                        "Rook" => new Rook(color),
                ...
                yield return new PlacedPiece(new Position(row, 0), baseLinePieces[0]);
                yield return new PlacedPiece(new Position(row, 1), baseLinePieces[1]);
                yield return new PlacedPiece(new Position(row, 2), baseLinePieces[2]);
                yield return new PlacedPiece(new Position(row, 3), baseLinePieces[3]);
                yield return new PlacedPiece(new Position(row, 4), baseLinePieces[4]);
                yield return new PlacedPiece(new Position(row, 5), baseLinePieces[5]);
                yield return new PlacedPiece(new Position(row, 6), baseLinePieces[6]);
                yield return new PlacedPiece(new Position(row, 7), baseLinePieces[7]);
            }
